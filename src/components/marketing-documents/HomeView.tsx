...
import {
  ...
  PenLine, Grid3X3, LayoutList, FolderOpen, Sparkles,
} from "lucide-react";
...
interface Props {
  ...
  onNewDoc: () => void;
  onOpenDoc: (doc: StrategyDoc) => void;
...
export default function HomeView({ documents, onNewDoc, onOpenDoc, onDeleteDoc, onRenameDoc }: Props) {
  ...
  const startRename = (doc: StrategyDoc, e: React.MouseEvent) => {
    e.stopPropagation();
    setMenuDocId(null);
    ...
    <div className="min-h-screen px-8 py-8">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2.5 mb-1">
          <div className="w-8 h-8 rounded-xl bg-[var(--gold-primary)]/15 border border-[var(--gold-primary)]/25 flex items-center justify-center">
            <Sparkles size={15} className="text-[var(--gold-primary)]" />
          </div>
          <h1 className="text-2xl font-semibold text-white">Strategy Document Generator</h1>
        </div>
        <p className="text-sm text-gray-500 mt-1">
          Generate professional strategy documents through AI-powered conversation.
        </p>
        ...
        <div className="flex items-center gap-6 mt-5">
          ...
          <div>
            ...
            <span className="text-sm text-gray-500 ml-2">Document Types</span>
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-28 text-center">
          ...
          <p className="text-white font-medium mb-1.5">
            {search ? "No documents match your search" : "No strategy documents yet"}
          </p>
          <p className="text-sm text-gray-500 mb-7">
            {search
              ? "Try a different keyword or document type"
              : "Start a conversation with the AI to generate your first strategy document"}
          </p>
          {!search && (
            <button
              onClick={onNewDoc}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ...
          <button
            onClick={onNewDoc}
            className="h-52 rounded-2xl border-2 border-dashed border-white/[0.07] hover:border-[var(--gold-primary)]/35 hover:bg-[var(--gold-primary)]/[0.03] flex flex-col items-center justify-center gap-3 text-gray-600 hover:text-[var(--gold-primary)] transition-all group"
          >
            ...
            <div className="text-center">
              <p className="text-sm font-medium">Generate New</p>
              <p className="text-xs opacity-60 mt-0.5">Via AI chat</p>
            </div>
          </button>
          ...
          {filtered.map(doc => {
            ...
            return (
              <div
                ...
                className="h-52 rounded-2xl bg-[#111111] border border-white/[0.06] hover:border-white/[0.14] hover:shadow-xl hover:shadow-black/50 cursor-pointer group relative flex flex-col overflow-hidden transition-all duration-200"
              >
                {/* Type-colored top bar */}
                <div className={`h-0.5 w-full bg-gradient-to-r ${style.bar} flex-shrink-0`} />
                ...
                <div className="flex-1 p-4 flex flex-col">
                    ...
                    <div className="relative" ref={menuDocId === doc.id ? menuRef : undefined}>
                      ...
                      {menuDocId === doc.id && (
                        <div className="absolute right-0 top-7 w-40 rounded-xl bg-[#1c1c1c] border border-white/[0.10] py-1 shadow-2xl shadow-black/80 z-20 animate-slideUp">
                          ...
                          >
                            <FolderOpen size={12} className="text-gray-500" /> Open
                          </button>
                          <button
                  ...
                  {/* Type badge */}
                  <span className={`self-start text-[10px] px-2 py-0.5 rounded-full border ${style.badge} mb-2`}>
                    {doc.type}
                  </span>
        ...
        <div className="rounded-2xl bg-[#111111] border border-white/[0.06] overflow-hidden">
          {/* Header row */}
          <div className="grid grid-cols-[1fr_160px_130px_96px_44px] px-5 py-3 border-b border-white/[0.06] bg-white/[0.02]">
            <span className="text-[11px] font-semibold uppercase tracking-widest text-gray-600">Name</span>
            <span className="text-[11px] font-semibold uppercase tracking-widest text-gray-600">Type</span>
          ...
          {filtered.map((doc, i) => {
            ...
            return (
              <div
                key={doc.id}
                onClick={() => renamingId !== doc.id && onOpenDoc(doc)}
                className={`grid grid-cols-[1fr_160px_130px_96px_44px] px-5 py-3.5 cursor-pointer group hover:bg-white/[0.03] transition-colors ${i > 0 ? "border-t border-white/[0.04]" : ""}`}
              >
                {/* Name */}
          ...
          <div
            onClick={onNewDoc}
            className="grid grid-cols-[1fr_160px_130px_96px_44px] px-5 py-3.5 border-t border-white/[0.04] cursor-pointer group hover:bg-white/[0.02] transition-colors"
          >
            <div className="flex items-center gap-3 text-gray-600 group-hover:text-[var(--gold-primary)] transition-colors">
              ...
              <span className="text-sm">Generate New Document</span>
            </div>
          </div>
        </div>
      )}
